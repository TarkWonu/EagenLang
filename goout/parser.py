...
def parse_program(src: str):
    ...
    def split_commas(s: str):
        ...
        for ch in s:
            if in_str:
                ...
                if ch == '"': in_str = True; buf.append(ch)
                elif ch == ',': out.append(''.join(buf).strip()); buf=[]
                else: buf.append(ch)
        tail=''.join(buf).strip()
    ...
    def parse_block():
        ...
        while i < len(body):
            ...
            if m:
                cond = m.group(1).strip(); i += 1
                then_body = parse_block()
                else_body = None
                if i < len(body) and body[i].strip().startswith("GO척결.디떨이 아니다?!"):
                    if not body[i].strip().endswith("{"):
                        raise GoOutSyntaxError("디떨이 아니다?! 는 { 로 열어야 합니다.")
                    i += 1
            ...
            if m:
                stmts.append(("print", m.group(1).strip())); i += 1; continue
            ...
            if m:
                ...
                stmts.append(("call", fname, args)); i += 1; continue
            ...
            if m:
                ...
                stmts.append(("assign", name, expr, is_decl)); i += 1; continue

            # 중괄호 단독 블록
            if line == '{':
                i += 1
                inner = parse_block()
                stmts.append(("block", inner)); continue
